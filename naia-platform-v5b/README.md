# Naia v5b — integrates Bernard's Paramètres redesign

**Live prototype: https://hydro-software.github.io/Designs-wireframes/naia-platform-v5b/**

Iteration of [`naia-platform-v5/`](../naia-platform-v5/) that folds in Bernard's
Paramètres rework documented in
[Designs-wireframes#1](https://github.com/hydro-software/Designs-wireframes/issues/1)
(originally drafted under `bl_draft/`). v5 is kept untouched as the comparison
reference for the conference scope discussion in
[platform#599](https://github.com/hydro-software/platform/issues/599).

## Changelog · v5 → v5b

Only one file changed vs. v5: `parametres.html`. Everything else (sidebar,
Tableau de bord, Production, Revenus, Données, Profil, MVP 0.9 toggle) is
identical to v5.

### Paramètres — Bernard's redesign (Designs-wireframes#1)
...
- **Composite indicators** — every composite formula now has a **constant** in 1st position, shown in the formula header as `constante + a · data_A + b · data_B`. Help text: "set the coefficient to 0 if the composite depends on only one data point".
- **Catégories de pertes — simplified to 2 radio/checkbox columns**: **Défaut type** (radio — 1 of 6 categories cocked as the default) and **Défaut sous-type** (radio — 1 of N per type cocked as default within that type), plus the existing **Affichage** checkbox column. Removed the old free-text/dropdown defaults.
- **Partage d'accès** — the "Retirer" icon button becomes a **Modifier** button that opens a modal where you can change the role *or* remove the access. Reduces accidental clicks.
- **Rename**: "Indicateurs contextuels" → "Indicateurs" throughout the page.

### What stayed v5

The other v5 changes (sidebar restructure, Tableau de bord, Production/Revenus chart with Indicateurs + Légende side panels, Données 3-pill, Profil with Paramètres utilisateur tile, MVP 0.9 toggle) are unchanged in v5b — they were not part of Bernard's review and the conference scope (platform#599) is still being discussed against the v5 surface.

### Bonus drafts in `bl_draft/` not yet folded in

Bernard also drafted `ingestion-donnees.html`, `ingestion-donnees-partage.html`, `popup-configuration-panneau.html` and `popup-configuration-panneau-partage.html`. The issue body only discusses Paramètres so those aren't integrated yet — they remain in `bl_draft/` for future iteration.
