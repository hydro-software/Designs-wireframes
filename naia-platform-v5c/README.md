# Naia v5c — adds Bernard's Données + chart-config popup

**Live prototype: https://hydro-software.github.io/Designs-wireframes/naia-platform-v5c/**

Iteration of [`naia-platform-v5b/`](../naia-platform-v5b/) that folds in the two
Bernard drafts (`ingestion-donnees.html` + `popup-configuration-panneau.html`)
that weren't covered by his original issue body and stayed unintegrated when v5b
shipped. All `bl_draft/` files from
[Designs-wireframes#1](https://github.com/hydro-software/Designs-wireframes/issues/1)
are now in a single integrated mockup. v5 stays untouched as the comparison
reference for the conference scope discussion in
[platform#599](https://github.com/hydro-software/platform/issues/599).

## Changelog · v5b → v5c

Two files changed vs. v5b: `data.html` (full replacement) and `production.html`
(chart-config overlay replaced). All other v5b changes (Paramètres) are carried
forward.

### Données (`data.html`) — Bernard's `ingestion-donnees.html`

- **"Compteur" → "Source de données"** — generic label that fits the three pill types (Électricité, Indicateur, Prix d'Électricité).
- **Unit shown as read-only inline text** (e.g. *"kWh · définie dans Paramètres → Compteurs · non modifiable"*) instead of a dropdown. The unit is bound to the source in Paramètres.
...
- **Default mode flipped to Évènementiel** (was Fréquentiel).
...
- **Centrale-tab "Toutes" points back to `data.html`** (Bernard's draft pointed to `ingestion-donnees.html`).
- The MVP 0.9 force on `DOMContentLoaded` in Bernard's draft is dropped — `data.html` now respects the user's MVP-tier choice via the global toggle.
- **"Imports récents" column** renamed `Compteur / Indicateur` → `Source de données`.

### Production — `#chart-config` overlay replaced with Bernard's `popup-configuration-panneau`

Reached from the toolbar **sliders** icon and from the bottom-right **wrench** icon. New structure:

- **Axe** section — three collapsible axis blocks: *Axe gauche* (Puissance / Énergie), *Axe droit 1* (Pluie), *Axe droit 2* (indicator chosen inline via a small select in the title). Each block holds a 4-column grid (Intraday / 30 jours / 12 mois / Années) with a per-resolution **unit selector + Min + Max** field and a per-axis **Réinitialiser** button.
...
- **Message WhatsApp** — placeholder block ("En cours de construction"), section collapsed by default.
- Sections + axis blocks use `data-open` / chevron rotation matching `parametres.html`.
- Modal width bumped to 920 px (`.modal-cfg`) to fit the per-resolution grids.

CSS additions live inline in `production.html` (`#chart-config`-scoped) so they don't leak into the rest of the app.

## Changelog · v5 → v5b (carried forward into v5c)

Only one file changed vs. v5 in v5b: `parametres.html`. Everything in v5c carries
the v5b changes forward.

### Paramètres — Bernard's redesign
...
- **New section "Flux de revenus"** — multiple revenue streams **cumulate** to form the global revenue. **Validity belongs to the whole period**, not to each individual flux. A history selector switches between periods.
...
- **Catégories de pertes — simplified to 2 radio/checkbox columns**: *Défaut type* (radio, 1 of 6) + *Défaut sous-type* (radio, 1 per type) + *Affichage* (checkbox).
- **Partage d'accès** — the "Retirer" icon becomes a **Modifier** link that opens a modal where you can change the role *or* remove the access.
- **Rename**: "Indicateurs contextuels" → "Indicateurs" throughout the page.

### What stayed v5

Sidebar restructure, Tableau de bord, Production/Revenus chart with Indicateurs + Légende side panels, Profil with Paramètres utilisateur tile, MVP 0.9 toggle are unchanged from v5 — the conference scope (platform#599) is still being discussed against the v5 surface.
